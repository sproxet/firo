        ...
        CAmount& changeAmount)
{
    ...
    CAmount fulfilled = 0;
    // Schnorr and range proofs are 956 bytes, the joinmint change is 157 bytes, and each P2SH recipeint is 22 bytes.
    ...
    if (coinControl.HasSelected()) {
        ...
        for (COutPoint selectedOutPoint: selectedOutPoints) {
            ...
            for (CMintMeta sigmaCoin: sigmaCoins) {
                ...
                if (sigmaCoin.isArchived || sigmaCoin.isUsed || !sigmaCoin.isSeedCorrect) {
                    throw WalletError(_("coin control selection includes an invalid, unknown, or public coin."));
                }

                if (sigmaCoin.nHeight == -1) {
                    throw WalletError(_("coin control selection includes coin with nHeight == -1."));
                }
